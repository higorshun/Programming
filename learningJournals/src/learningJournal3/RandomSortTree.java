...
/**
 * This program makes a random binary sort tree containing 1023 random * real numbers. It then computes the height of
 * the tree and the
...
public class RandomSortTree {
    ...
    static void treeInsert(double x) {
        ...
        TreeNode runner = null; // Runs down the tree to find a place for newItem. runner = root; // Start at the root.
        while (true) {
            if (x < runner.item) {
// Since the new item is less than the item in runner,it belongs in the left subtree of runner. If there
...
// Since the new item is greater than or equal to the // item in runner, it belongs in the right subtree of
// runner. If there is an open space at runner.right,
// add a new node there. Otherwise, advance runner // down one level to the right.
    ...
    /**
     * When called as sumOfLeafDepths(root,0), this will compute the * sum of the depths of all the leaves in the
     * tree to which root
...
// The node is a leaf, and there are no subtrees of node, so // the sum of the leaf depth is just the depths of this
// node.
            return depth;
        } else {// The node is not a leaf. Return the sum of the // the depths of the leaves in the subtrees.
    ...
    /**
     * When called as maximumLeafDepth(root,0), this will compute the * max of the depths of all the leaves in the
     * tree to which root
     * points. When called recursively, the depth parameter gives
...
// The node is a leaf, so the maximum depth in this // subtree is the depth of this node (the only leaf
// that it contains).
            return depth;
...
// represents the maximum in the tree overall.
    ...
    public static void main(String[] args) {
        root = null; // Start with an empty tree. Root is a global // variable, defined at the top of the class.
...
// Display the results.
    ...
    static class TreeNode {
        ...
        TreeNode(double x) {// Constructor. Make a node containing x. item = x;
        }
    } // end class TreeNode
