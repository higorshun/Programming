...
public class PhoneDirectory {
     ...
     * If found, the index * of the pair in the data array is returned.
     * If no pair contains the * given name, then the return value is -1.
     * This private method is * used internally in getNumber() and putNumber().
     */

    private int find(String name) {
        for (int i = 0; i < dataCount; i++) {
            if (data[i].name.equals(name)) return i;// The name has been found in position i.
        }
    ...
    public String getNumber(String name) {
        ...
        if (position == -1) return null; // There is no phone entry for the given name.
        else return data[position].number;
    }

    /**
     * Associates a given name with a given phone number.
     * If the name * already exists in the phone directory, then the new number replaces
